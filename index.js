...
.text('Предложить тему', 'topic_btn').row()
.text('Рассказать о проекте', 'project_btm').row()
...
await ctx.callbackQuery.message.editText('Чем Вы занимаетесь?\n\nРасскажите в нескольких предложениях о проекте и оставьте контакт для связи.', { reply_markup: backKeyboard,
});
await ctx.api.editMessageText 
await ctx.answerCallbackQuery()
...
await ctx.callbackQuery.message.editText('Любые идеи и конструктивная критика приветствуются. Помогите нам стать лучше!', { reply_markup: backKeyboard,
});
await ctx.api.editMessageText 
await ctx.answerCallbackQuery()
...
await ctx.callbackQuery.message.editText('Привет, это бот «Цифровой истории»!\n\nСюда можно отправить любые предложения, новости и пожелания. \n\nО чем хотите написать?', { reply_markup: startKeyboard,
});
await ctx.api.editMessageText 
await ctx.answerCallbackQuery()
...
bot.command('improvements',async (ctx) => {
  await  ctx.reply('Любые идеи и конструктивная критика приветствуются. Помогите нам стать лучше!', {reply_markup: backKeyboard,});
  getButtonMetaData('improve_btm');
});
...
{
  await ctx.reply('Спасибо, что нашли время и написали нам! \n \n Хотите еще что-нибудь написать?', {reply_markup: startKeyboard,});
}
...
bot.on("message", async (ctx) => {
  ...
  const mesInfo = [[message.chat.id, message.chat.username , message.chat.first_name,message.chat.last_name,date,message.text]];
  ...
  (async () =>
  {
    ...
    try 
    {
      const lastMessageNumber = await database.getMessagesNumber(auth,sheetName);
